...
data_by_model<- DA_data_df %>% 
  filter(model %in% c("GFDLESM2")) %>% group_by(model) %>% group_split()
...
exceedance_prob <- function(data_to_fit_model, obs2022, period) {
  ...
  filtered_data <- data_to_fit_model %>%
    filter(between(year, period[1], period[2]))
  
  # Check if there are enough data points
  if (nrow(filtered_data) < 3) {
    stop("Not enough data points to fit the GEV model.")
  }
  
  # Fit GEV and calculate exceedance probabilities with error handling
  fit_results <- tryCatch({
    fitGEV <- fgev(filtered_data$tx5d_ano, std.err = FALSE)  # Fit GEV without standard errors
    ecdffit <- ecdf(filtered_data$tx5d_ano)
    list(
      fitGEV = fitGEV,
      ecdffit = ecdffit
    )
  }, error = function(e) {
    message("GEV fitting failed: ", e$message)
    return(NULL)
  })
  
  if (is.null(fit_results)) {
    return(filtered_data %>% mutate(
      exceedProb = NA,
      exceedProb_obs2022 = NA,
      exceedProb_EmpDist = NA,
      exceedProb_obs2022_EmpDist = NA,
      tx5d_ano_arrenged = sort(tx5d_ano)
    ))
  }
  
  # Calculate exceedance probabilities using the GEV fit and empirical distribution
  filtered_data <- filtered_data %>%
    mutate(
      exceedProb = 1 - pgev(sort(tx5d_ano), 
                            fit_results$fitGEV$estimate["loc"],
                            fit_results$fitGEV$estimate["scale"], 
                            fit_results$fitGEV$estimate["shape"]),
      exceedProb_obs2022 = 1 - pgev(obs2022, 
                                    fit_results$fitGEV$estimate["loc"],
                                    fit_results$fitGEV$estimate["scale"], 
                                    fit_results$fitGEV$estimate["shape"]),
      exceedProb_EmpDist = 1 - fit_results$ecdffit(sort(tx5d_ano)),
      exceedProb_obs2022_EmpDist = 1 - fit_results$ecdffit(obs2022),
      tx5d_ano_arrenged = sort(tx5d_ano)
    )
...
cFactPeriod<-c(1861,1949) # define counter factual world; CHANGE HERE IF REQUIRED
...
cFact_stats <- data_by_model %>% lapply(exceedance_prob, obs2022 = DA_obs2022_df$tx5d_ano, period = cFactPeriod) %>% bind_rows() 
cFact_stats<- cFact_stats[!names(cFact_stats) %in% c('fitGEV', 'ecdffit')];  str(cFact_stats)

Fact_stats <- data_by_model %>% lapply(exceedance_prob, obs2022 = DA_obs2022_df$tx5d_ano, period = FactPeriod) %>% bind_rows() 
Fact_stats<- Fact_stats[!names(Fact_stats) %in% c('fitGEV', 'ecdffit')];  str(Fact_stats)

Fut_stats <- data_by_model %>% lapply(exceedance_prob, obs2022 = DA_obs2022_df$tx5d_ano, period = FutPeriod) %>% bind_rows() 
...
exceedance_probs <- cFact_stats %>%
  filter(model %in% c("GFDLESM2")) %>% group_by(model) %>%
  summarise(cFact_prob = mean(exceedProb_obs2022, na.rm = TRUE),
            .groups = 'drop') %>% left_join( Fact_stats %>% filter(model %in% c("GFDLESM2")) %>%
                                               group_by(model) %>% summarise(Fact_prob = mean(exceedProb_obs2022, na.rm = TRUE), .groups = 'drop'),by = "model") %>%
  left_join(Fut_stats %>% filter(model %in% c("GFDLESM2")) %>% group_by(model) %>% 
...
exceedance_probs_EmpDist <- cFact_stats %>% filter(model %in% c("GFDLESM2")) %>%
  group_by(model) %>% summarise( cFact_prob_EmpDist = mean(exceedProb_obs2022_EmpDist, na.rm = TRUE),
                                 .groups = 'drop') %>% left_join( Fact_stats %>% filter(model %in% c("GFDLESM2")) %>%
                                                                  ...
                                                                  by = "model") %>% left_join(Fut_stats %>% filter(model %in% c("GFDLESM2")) %>%
...
p2 <- cFact_stats %>% filter(model %in% c("GFDLESM2")) %>%
  ggplot(aes(x = tx5d_ano_arrenged)) +
  ...
  geom_vline(xintercept = DA_obs2022_df$tx5d_ano, color = "purple", linetype = "dashed") +  # Vertical line for observed 2022
  labs(title = "Exceedance probability distribution (based on GEV)", x = "tx5d_ano", y = "Exceedance probability (EP)", color = "Legend") +  # Legend title
  ...
  # Add EP values
  geom_text(data = exceedance_probs, aes(x = max(cFact_stats$tx5d_ano, na.rm = TRUE) * 0.5, 
                                         y = Inf, label = annotation_text), hjust = 0, vjust = 1.5, size = 4, color = c("black"), 
...
p3 <- cFact_stats %>% filter(model %in% c("GFDLESM2")) %>%
  ggplot(aes(x = tx5d_ano_arrenged)) +
  facet_wrap(~model, scales = "free_y") +
  ...
  geom_vline(xintercept = DA_obs2022_df$tx5d_ano, color = "purple", linetype = "dashed") +  # Vertical line for observed 2022
  labs(title = "Exceedance probability distribution (based on Empirical distribution)", x = "tx5d_ano", y = "Exceedance probability (EP)", color = "Legend") +  # Legend title
  scale_color_manual(name="Legend",values = manual_colors, labels = c(cFact_label, Fact_label, Fut_label)) +  # Apply manual colors
  theme_minimal() +
  theme(legend.text = element_text(size = 12))+
  # Add EP values
  geom_text(data = exceedance_probs_EmpDist, aes(x = max(Fut_stats$tx5d_ano, na.rm = TRUE) * 0.5, 
                                                 y = Inf, label = annotation_text_EmpDist), 
