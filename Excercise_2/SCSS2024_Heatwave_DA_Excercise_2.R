...
data_by_model<- DA_data_df %>% 
  filter(model %in% c("CanESM5","CESM2")) %>% group_by(model) %>% group_split()
...
exceedance_prob<-function(data_to_fit_model, obs2022, period) {
  ...
  data_to_fit_model %>%
    ...
    mutate(
      fitGEV = list(fgev(tx5d_ano)), # Fit GEV for the filtered period
      ecdffit = list(ecdf(tx5d_ano)),
      exceedProb = 1 - pgev(sort(tx5d_ano),  fitGEV[[1]]$estimate["loc"],fitGEV[[1]]$estimate["scale"], fitGEV[[1]]$estimate["shape"]),
      exceedProb_obs2022 = 1 - pgev(obs2022,  fitGEV[[1]]$estimate["loc"],fitGEV[[1]]$estimate["scale"], fitGEV[[1]]$estimate["shape"]),
      # Calculate exceedance probability using empirical distribution
      exceedProb_EmpDist = 1 - ecdffit[[1]](sort(tx5d_ano)),
      
      # Calculate exceedance probability for the observed value using empirical distribution
      exceedProb_obs2022_EmpDist = 1 - ecdffit[[1]](obs2022),
      tx5d_ano_arrenged = sort(tx5d_ano)) #%>%  
}

cFactPeriod<-c(1850,1950) # define counter factual world; CHANGE HERE IF REQUIRED
FactPeriod<-c(1981,2022) # define factual (or current) world; CHANGE HERE IF REQUIRED
FutPeriod<-c(2055,2100) # define future world; CHANGE HERE IF REQUIRED
FutPeriod<-c(2022,2055) # define future world; CHANGE HERE IF REQUIRED
...
exceedance_probs <- cFact_stats %>%
  filter(model %in% c("CanESM5", "CESM2")) %>% group_by(model) %>%
  summarise(cFact_prob = mean(exceedProb_obs2022, na.rm = TRUE),
            .groups = 'drop') %>% left_join( Fact_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>%
                                               group_by(model) %>% summarise(Fact_prob = mean(exceedProb_obs2022, na.rm = TRUE), .groups = 'drop'),by = "model") %>%
  left_join(Fut_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>% group_by(model) %>% 
...
exceedance_probs_EmpDist <- cFact_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>%
  group_by(model) %>% summarise( cFact_prob_EmpDist = mean(exceedProb_obs2022_EmpDist, na.rm = TRUE),
            .groups = 'drop') %>% left_join( Fact_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>%
                          ...
                          by = "model") %>% left_join(Fut_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>%
...
p2 <- cFact_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>%
...
p3 <- cFact_stats %>% filter(model %in% c("CanESM5", "CESM2")) %>%
  ggplot(aes(x = tx5d_ano_arrenged)) +
  facet_wrap(~model, scales = "free_y") +
...
# look at the stadardized anonmaly as well
# it is prefered to work at regional level